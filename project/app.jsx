...
const GLYPHS = {
  ...
  'bullwhip': <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 8c1.5 0 1.5-3 3-3s1.5 4 3 4 1.5-6 3-6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /><path d="M2 8c1 0 1 5 2.5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity="0.5" /></svg>,
...
const SunIcon = () => <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="3" stroke="currentColor" strokeWidth="1.5" /><path d="M8 1v1.5M8 13.5V15M1 8h1.5M13.5 8H15M3 3l1 1M12 12l1 1M13 3l-1 1M4 12l-1 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>;
const MoonIcon = () => <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M13.5 9.5A6 6 0 0 1 6.5 2.5 6 6 0 1 0 13.5 9.5z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" /></svg>;
...
function App() {
  ...
  useEffect(() => { document.documentElement.setAttribute('data-theme', theme); localStorage.setItem('econsim.theme', theme); }, [theme]);
  useEffect(() => { localStorage.setItem('econsim.active', active); }, [active]);

  const sim = SIMS[active];
  const Comp = sim.Component;
  const idx = SIM_ORDER.indexOf(active) + 1;

  const tabs = [{ id: 'sim', label: 'Simulador', n: '01' }, { id: 'explain', label: 'Explicación', n: '02' }, { id: 'theory', label: 'Teoría', n: '03' }];
  ...
  return (
    <div className="app">
      <aside className="sidebar">
        ...
        <nav className="nav">
          {SIM_ORDER.map((id, i) => (
            <button key={id} className={"nav-item" + (active === id ? " active" : "")} onClick={() => { setActive(id); setTab('sim'); }}>
      ...
      <main className="main">
        <div className="main-inner">
          ...
          <Comp tab={tab === 'sim' ? 'sim' : tab === 'explain' ? 'explain' : 'theory'} key={active + tab} />
        </div>
      </main>
    </div>
  );
